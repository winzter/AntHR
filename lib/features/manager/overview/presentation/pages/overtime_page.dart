...
class _OvertimePageState extends State<OvertimePage> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: Scaffold(
        appBar: appBarOverview(context,"ข้อมูลค่าล่วงเวลา"),
        body: Stack(children: [
          Container(
                  ...
                  Expanded(
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        children: [
                          MonthYearPicker(
                            bloc: overviewBloc, title: "ข้อมูลค่าล่วงเวลา",
                          ),
                          Padding(
                          ...
                          BlocBuilder<OverviewBloc, OverviewState>(
                            builder: (context, state) {
                              return ExpenseBox(
                                title: 'ทำงานล่วงเวลา',
                                value: state.overtimeData != null?(state.overtimeData!.otTotal!.the15!.hourTotal! +  state.overtimeData!.otTotal!.the2!.hourTotal! + state.overtimeData!.otTotal!.the3!.hourTotal!):0,
                                svgPath:
                          ...
                          BlocBuilder<OverviewBloc, OverviewState>(
                            builder: (context, state) {
                              return ExpenseBox(
                                ...
                                value: state.overtimeData != null?(state.overtimeData!.otTotal!.the15!.payTotal! +  state.overtimeData!.otTotal!.the2!.payTotal! + state.overtimeData!.otTotal!.the3!.payTotal!):0,
                                svgPath:
                          ...
                          BlocBuilder<OverviewBloc, OverviewState>(
                            builder: (context, state) {
                              return OtChart(overtimeData: state.overtimeData, date: state.date,);
                            },
                          ),
                          BlocBuilder<OverviewBloc, OverviewState>(
                            builder: (context, state) {
                              return Top5Overtime36(overtimeData: state.overtimeData,);
                            },
                          ),
                        ],
                      ),
