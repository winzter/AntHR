...
class OverviewBloc extends Bloc<OverviewEvent, OverviewState> {
  ...
  OverviewBloc({
    ...
    on<GetDepartmentData>((event, emit) async {
      ...
      res.fold((l) => emit(state.copyWith(status: FetchStatus.failed)), (r) {
        for (var i in r) {
          ...
          if (i.idDepartment != null && i.departmentName != null) {
            if(i.divisionName != null){
              departmentName[i.idDepartment!] = "${i.departmentName!} (${i.divisionName})";
            }
            else{
              departmentName[i.idDepartment!] = i.departmentName!;
            }
            log("${i.departmentName!} (${i.divisionName})");
          } else {
    ...
    on<GetOverviewData>((event, emit) async {
      ...
      res.fold(
          (ErrorMessage l) => emit(state.copyWith(status: FetchStatus.failed)),
    ...
    on<GetOvertimeData>((event, emit) async {
      ...
      res.fold(
          ...
          (r) => emit(
              state.copyWith(status: FetchStatus.success, overtimeData: r,selectedSectionName: state.selectedSectionName,
                selectedDivisionName: state.selectedDivisionName,
                selectedDepartmentName: state.selectedDepartmentName,)));
    });
    ...
    on<GetWorkingTimeData>((event, emit) async {
      ...
      res.fold(
          (l) => emit(state.copyWith(status: FetchStatus.failed)),
          (r) => emit(
              state.copyWith(status: FetchStatus.success, workingTimeData: r,selectedSectionName: state.selectedSectionName,
                selectedDivisionName: state.selectedDivisionName,
                selectedDepartmentName: state.selectedDepartmentName,)));
    });
    ...
    on<GetCostData>((event, emit) async {
      ...
      res.fold(
          (l) => emit(state.copyWith(status: FetchStatus.failed)),
          (r) =>
              emit(state.copyWith(status: FetchStatus.success, costData: r,selectedSectionName: state.selectedSectionName,
                selectedDivisionName: state.selectedDivisionName,
                selectedDepartmentName: state.selectedDepartmentName,)));
    });
