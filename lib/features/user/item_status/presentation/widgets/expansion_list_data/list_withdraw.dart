...
class ListWithdrawItems extends StatefulWidget {
  ...
  const ListWithdrawItems({super.key, required this.index, required this.withdrawData, required this.leaveData});
...
class _ListWithdrawItemsState extends State<ListWithdrawItems> {
  ...
  Widget build(BuildContext context) {
    ...
    LeaveEntity data = widget.leaveData
        .firstWhere((element) => element.idLeave == widget.withdrawData.idLeave);
    return Padding(
                ...
                ListTile(
                  ...
                  trailing: Text(
                    "${DateFormat('dd/MM/yyyy').format(data.start!)}"
                        "\n${item.isFullDay(data.start!, data.end!)}",
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      data.approveDate == null
                                          ? "-"
                                          : "${DateFormat('dd/MM/yyyy').format(DateTime.parse(data.approveDate!))}"
                                          "\n${DateFormat('HH:mm').format(DateTime.parse(data.approveDate!))}",
                                      style: const TextStyle(
                                          fontSize: 14,
                                          color: Color(0xff757575)),
                                      textAlign: TextAlign.end,
